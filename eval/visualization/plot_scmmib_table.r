...
plot_scmmib_summary<-function(data,row_info,column_info,palettes,rank=FALSE,rect_normalize=TRUE,extend_figure=FALSE){
  ...
  cat("DEBUG text section: ind_text =", ind_text, "\n")
  cat("DEBUG: ncol(data) =", ncol(data), ", nrow(data) =", nrow(data), "\n")
  cat("DEBUG: column_info$id =", column_info$id, "\n")
  cat("DEBUG: column_info$geom =", column_info$geom, "\n")
  dat_mat <- as.matrix(data[, ind_text],drop=F)
  cat("DEBUG: after subset - ncol(dat_mat) =", ncol(dat_mat), ", nrow(dat_mat) =", nrow(dat_mat), "\n")
  cat("DEBUG: colnames(dat_mat) =", colnames(dat_mat), "\n")
  cat("DEBUG: as.vector(dat_mat) length =", length(as.vector(dat_mat)), "\n")
  cat("DEBUG: rep(colnames(dat_mat), each = nrow(dat_mat)) length =", length(rep(colnames(dat_mat), each = nrow(dat_mat))), "\n")
